...
(function (global) {
  ...
  function renderBackendError(message, backendUrl) {
    return `
    <div class="auth-screen">
      <div class="auth-card">
        <h1>Controle de Qualidade — Radioterapia</h1>
        <p class="form-error">${esc(message)}</p>
        <p class="muted small">Este app agora depende do backend para tudo (não só para analisar DICOM) — usuários, equipamentos, rotinas e resultados ficam salvos lá. Confirme que ele está rodando:</p>
        <pre class="raw-json">cd backend
pip install -r requirements.txt
uvicorn main:app --host 0.0.0.0 --port 8420</pre>
        <label>URL do backend
          <input type="text" id="backend-url-input-boot" value="${esc(backendUrl)}" />
        </label>
        <button type="button" class="btn btn-primary btn-block mt" data-action="retry-backend-connection">Tentar de novo</button>
      </div>
    </div>`;
  }
    ...
    const resultRows = results
      .map((res) => {
        ...
        <tr>
          ...
          <td>${valuesSummary}${metrics.length > 3 ? " …" : ""} ${res.autoGenerated ? '<span class="badge badge-muted" title="Gerado pelo observador de pasta">⚙ Automático</span>' : ""}</td>
  ...
  function watchFolderPanelHtml(routine, watchFolders) {
    const rows = watchFolders
      .map(
        (wf) => `
      <tr>
        <td><code>${esc(wf.folderPath)}</code>${wf.lastError ? `<div class="form-error small">${esc(wf.lastError)}</div>` : ""}</td>
        <td>${wf.active ? '<span class="badge badge-success">Ativa</span>' : '<span class="badge badge-muted">Pausada</span>'}</td>
        <td class="row-actions">
          <button class="icon-btn" data-action="toggle-watch-folder" data-id="${wf.id}" data-active="${wf.active ? "0" : "1"}" title="${wf.active ? "Pausar" : "Reativar"}">${wf.active ? "⏸" : "▶"}</button>
          <button class="icon-btn" data-action="delete-watch-folder" data-id="${wf.id}" title="Remover">🗑</button>
        </td>
      </tr>`
      )
      .join("");
    ...
    return `
      <div class="panel">
        <div class="panel-header"><h3>Pasta observada (análise automática)</h3></div>
        <p class="muted small">O backend fica observando a(s) pasta(s) abaixo (no computador onde ele roda) e, a cada arquivo novo, analisa e grava o resultado sozinho no histórico — mesmo sem o navegador aberto. Depois é só entrar aqui para conferir e aprovar.</p>
        ${
          watchFolders.length
            ? `<table class="mini-table"><thead><tr><th>Pasta</th><th>Status</th><th></th></tr></thead><tbody>${rows}</tbody></table>`
            : ""
        }
        <form id="watch-folder-form" class="stacked-form mt" data-routine-id="${routine.id}">
          <label>Caminho completo da pasta (no computador onde o backend roda)
            <input type="text" name="folderPath" placeholder="Ex.: Z:\\CQ\\2 Semanal\\CQ_MLC\\MOSAIQ Export" required />
          </label>
          <div class="form-actions" style="justify-content:flex-start;">
            <button type="submit" class="btn">+ Observar esta pasta</button>
          </div>
        </form>
      </div>`;
  }
      ...
      <div class="result-detail">
        ...
        ${
          result.analyzedWithPylinac
            ? `<p class="muted small">⚙ Calculado pelo pylinac a partir de: ${esc((result.sourceFiles || []).join(", "))}${result.autoGenerated ? " (via pasta observada, sem intervenção manual)" : ""}</p>`
            : ""
        }
  ...
  function renderBackup(db, backendUrl, autoBackupSetting) {
    ...
    return `
      ...
      <div class="panel">
        <div class="panel-header"><h3>Servidor (backend)</h3></div>
        <p class="muted">Todos os dados do sistema (usuários, equipamentos, rotinas e resultados) ficam no banco do backend, não neste navegador — por isso ele precisa estar rodando (veja <code>backend/README.md</code> no repositório) para o app funcionar. Informe abaixo o endereço onde ele está disponível.</p>
        <label>URL do backend
      ...
      <div class="panel">
        ...
        <p class="muted">Baixa uma cópia de tudo que está no banco do backend (equipamentos, rotinas, resultados e usuários) em um arquivo JSON. Exporte periodicamente para ter uma cópia de segurança do arquivo <code>backend/rtqc.db</code>.</p>
      ...
      <div class="panel">
        ...
        <p class="muted">Importar um backup <b>substitui todos os dados atuais do backend</b>.</p>
  ...
  function renderHelp() {
    return `
      ...
      <div class="panel">
        ...
        <p>Esta é uma plataforma de <b>controle de qualidade em radioterapia</b>. A interface é HTML/JS e roda no navegador, mas todos os dados (usuários, equipamentos, rotinas e resultados) ficam salvos no <b>backend</b> — um servidor Python que roda no seu computador (ou em um servidor da sua rede) e que também é quem executa as análises do pylinac.</p>
        <ul class="plain-list">
      ...
      <div class="panel">
        <div class="panel-header"><h3>Pasta observada — análise 100% automática</h3></div>
        <p>Para testes com upload de arquivo (ex.: Picket Fence), é possível configurar uma <b>pasta local do computador onde o backend roda</b> para ser observada continuamente: qualquer arquivo novo que aparecer lá é analisado automaticamente pelo pylinac e o resultado é gravado direto no histórico da rotina — <b>sem precisar abrir o navegador</b> para isso acontecer. Configure em cada rotina, na seção "Pasta observada (análise automática)".</p>
        <p class="muted small">O arquivo processado é movido para uma subpasta <code>processado_cq</code> (ou <code>falha_cq</code>, se a análise falhar) dentro da própria pasta observada, para não ser analisado de novo.</p>
        <p class="muted small">Resultados assim gerados ainda precisam da aprovação manual (usuário + senha) — a automação cobre a análise, não a validação clínica.</p>
      </div>
      <div class="panel">
        <p class="muted small">O login/senha funciona como registro de responsabilidade (assinatura eletrônica), não como segurança de nível hospitalar com múltiplos servidores. Faça backups regulares (menu Backup) do arquivo <code>backend/rtqc.db</code>.</p>
      </div>
    `;
  }
